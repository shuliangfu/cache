/**
 * @module @dreamer/cache/client/adapters/local-storage
 *
 * @fileoverview localStorage 缓存适配器
 */

import type { CacheAdapter, CacheItem } from "./base.ts";

/**
 * localStorage 缓存适配器配置选项
 */
...
export class LocalStorageAdapter implements CacheAdapter {
  ...
  constructor(options: LocalStorageAdapterOptions = {}) {
    if (typeof localStorage === "undefined") {
      throw new Error("localStorage 不可用，请在浏览器环境中使用");
    }
  ...
  set(key: string, value: unknown, ttl?: number, tags?: string[]): void {
    try {
      ...
      if (error instanceof Error && error.name === "QuotaExceededError") {
        throw new Error("localStorage 存储空间已满");
      }
      throw error;
    }
  }
